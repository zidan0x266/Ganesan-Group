...
def main():
    ...
    for i in range(10001):
        ...
        _, _, g1, g2 = getgroups(i, ASSOac, ASSOal)
        b1 = getbridges(g1)
        b2 = getbridges(g2)
        bs1, bt1 = getarchs(b1, 15)
        bs2, bt2 = getarchs(b2, 15)
        Btwins1.append(np.sum(bt1) + np.sum(bt2))# / len(n1))
        Bsingle1.append(np.sum(bs1) + np.sum(bs2))# / len(n1))
        Btwins2.append(np.sum(bt2))# / len(n2))
        Bsingle2.append(np.sum(bs2))# / len(n2))
    anaPrint('Bridge_total2', Btwins1, Bsingle1, Btwins2, Bsingle2)
