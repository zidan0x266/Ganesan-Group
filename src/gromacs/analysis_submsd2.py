...
def main():
    ...
    for start in START:
        ...
        for time in range(start, tau + start + 1):
            tmpstateg1 = np.zeros(len(aP))
            tmpstateg2 = np.zeros(len(aP))
            tmpstateg3 = np.zeros(len(aP))
            if firstframe:  # marker the inital status of the ion group
                atomg1, atomg2 = getgroups(time, ASSOac, ASSOal)
                atomg3 = np.setdiff1d(lP, atomg1 + atomg2)
                for atom in atomg1:
                    stateg1[atom] = 1
                for atom in atomg2:
                    stateg2[atom] = 1
    ...
    msdPrint('subMSD2', xdata, msd0, msd1, msd2, msd3)
