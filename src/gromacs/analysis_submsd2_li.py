...
def main():
    ...
    aL = uta.select_atoms('type opls_406')  # fetch lithiums
    #lL = np.linspace(0, len(aL) - 1, len(aL), dtype = int)  # create the atom index
    h5 = h5py.File('lithiums.h5', 'w')
    dtype = h5py.special_dtype(vlen = np.dtype('int32'))
    ds_LT1 = get_or_create_dataset(h5, 'LT1', dtype)
    ds_LT2 = get_or_create_dataset(h5, 'LT2', dtype)   
    ...
    for time in range(frames):
        ...
        cocoli, solo = ligroups(time, ASSOac, ASSOal)
        ds_append(ds_LT1, cocoli)
        ds_append(ds_LT2, solo)
    h5.close()
    lL = h5py.File('lithiums.h5', 'r')
    lt1 = rawLoad(lL, 'LT1')
    lt2 = rawLoad(lL, 'LT2')
    START = np.linspace(0, 8000, 1001, dtype = int)
    tau = 2000
    MSDg0, MSDg1, MSDg2 = [], [], []
    for start in START:
        print(start)
        ...
        for time in range(start, tau + start + 1):
            tmpstateg1 = np.zeros(len(aL))
            tmpstateg2 = np.zeros(len(aL))
            if firstframe:  # marker the inital status of the ion group
                atomg1, atomg2 = lt1[time], lt2[time]
                for atom in atomg1:
                    stateg1[atom] = 1
                for atom in atomg2:
                    stateg2[atom] = 1
    ...
    msdPrint('subMSD2_Li', xdata, msd0, msd1, msd2)
