...
def define_atom_types(uta, anion_type = "type PF", cation_type = "type IM"):
    """ 
...
def create_position_arrays(uta, anions, cations, times):
    """
    ...
    for ts in uta.trajectory:
        anion_positions[time, :, :] = anions.positions - uta.atoms.center_of_mass()
        cation_positions[time, :, :] = cations.positions - uta.atoms.center_of_mass()
        time += 1
...
def main():
    ...
    anion_positions, cation_positions = create_position_arrays(uta, anions, cations, times)
    msds_all = compute_all_Lij(cation_positions, anion_positions, times)
    outPrint('msd_self_ag1', times, msds_all[3])
    outPrint('msd_self_ag2', times, msds_all[1])
    outPrint('msd_dis_ag11', times, msds_all[2])
    outPrint('msd_dis_ag22', times, msds_all[0])
    outPrint('msd_dis_ag12', times, msds_all[4])
