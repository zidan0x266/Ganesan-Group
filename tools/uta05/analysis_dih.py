...
def main():
    uta = mda.Universe('md.tpr', 'data.xtc')
    nframes = len(uta.trajectory)
    #index = ["resname HPEO and name C1", "resname HPEO and name C2", "resname HPEO and name O1", "resname MPEO and name C1", "resname MPEO and name C2", "resname MPEO and name O1", "resname TPEO and name C1", "resname TPEO and name C2", "resname TPEO and name O1", "resname HPIG and name C1", "resname HPIG and name C2", "resname HPIG and name O1"]
    backbone = uta.select_atoms('name C1 or name C2 or name O1')  # select atoms only on backbone
    dihindex = []
    for i, dih in enumerate(backbone.dihedrals):
        if dih.atoms[0].name == 'C1' and dih.atoms[1].name == 'C2' and dih.atoms[2].name == 'O1' and dih.atoms[-1].name == 'C1':
            dihindex.append(i)
        elif dih.atoms[0].name == 'C2' and dih.atoms[1].name == 'O1' and dih.atoms[2].name == 'C1' and dih.atoms[-1].name == 'C2':
            dihindex.append(i)
        elif dih.atoms[0].name == 'O1' and dih.atoms[1].name == 'C1' and dih.atoms[2].name == 'C2' and dih.atoms[-1].name == 'O1':
            dihindex.append(i)
    cosvals = np.zeros((nframes, len(dihindex)))
    for ts in uta.trajectory:
        frame = ts.frame
        cosvals[frame] = np.cos(backbone.dihedrals.values()[dihindex])
        if frame % 10 == 0:
            print('Now processing percentage: {:5.3f}'.format(float(frame) / (nframes - 1)))
    acfphi = finalGetdx_i(cosvals, 0, 1, 48)
