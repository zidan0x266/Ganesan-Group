...
def main():
    uta = mda.Universe("cg_topol.tpr", "unwrap.xtc")
    aL = uta.select_atoms('all')  # all atoms
    aP = uta.select_atoms('type PF')  # anion PF6
    aN = uta.select_atoms('type IM')  # cation IM
    axis = 0
    al_unwrap = []
    ap_unwrap = []
    an_unwrap = []
    ...
    for ts in uta.trajectory:
        if ts.frame >= 0:
            ...
            ap_unwrap.append(aP.positions[:,axis] - com[axis])
            an_unwrap.append(aL.positions[:,axis] - com[axis])
    dt = np.linspace(0, 85000, 85001)
    dxap = finalGetdx_i(ap_unwrap, 0, 1, 48)
    dx_ap = np.average(dxap, axis = 1)
    dpPrint('displ_aPF', dt, dx_ap)
    dxan = finalGetdx_i(an_unwrap, 0, 1, 48)
    dx_an = np.average(dxan, axis = 1)
    dpPrint('displ_aIM', dt, dx_an)
