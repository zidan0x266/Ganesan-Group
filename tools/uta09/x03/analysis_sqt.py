...
def main():
    uta = mda.Universe("../md.tpr", "unwrapped.xtc")
    ...
    scatter_factor = dict([(1, -3.7406), (12, 6.6511), 
                           ...
                           (19, 5.6540), (7, -1.9000), 
                           (32, 2.8040)])
    scatter_length = [scatter_factor[int(round(atom_mass))] for atom_mass in poly.masses]
    # initialization
    ...
    for qindex in range(n_max):  # go through all q values
        ...
        for frame in uta.trajectory:  # go through all frames
            ...
            #    qr = qx * frame.positions[aindex][0] + qy * frame.positions[aindex][1] + qz * frame.positions[aindex][2]
            ...
            qr = np.sum(poly.positions * q_cur, axis = 1)
            time = frame.frame
            cossum[time] = np.sum(scatter_length * np.cos(qr))
            sinsum[time] = np.sum(scatter_length * np.sin(qr))
        #a = int(q[qindex] * 100)
